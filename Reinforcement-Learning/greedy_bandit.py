...
for i in range(iterations):
    ...
    for j in range(plays):
        ...
        # Random benchmark with which to compare epsilon
        benchmark = np.random.random()
        
        # Pick random action if the list is empty or if it's time for a random action
        if not play or benchmark < epsilon:
            pick = np.random.randint(0, num_actions)
            play = True # played once, now only pick if bench < epsilon
